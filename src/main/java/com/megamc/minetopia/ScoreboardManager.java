...
public class ScoreboardManager {
    ...
    private boolean updateOnPlayerJoinQuit;
    private boolean updateOnEconomyChanges;
    private boolean updateOnLevelChanges;
    private boolean updateOnHealthHungerChanges;
    ...
    public void loadConfig() {
        ...
        // Load economy formatting settings
        this.economyEnabled = plugin.getConfig().getBoolean("scoreboard.economy.enabled", true);
        this.formattedFormat = plugin.getConfig().getString("scoreboard.economy.formats.formatted", "%.2f");
        this.shortFormat = plugin.getConfig().getString("scoreboard.economy.formats.short", "%.0f");
        this.currencyFormat = plugin.getConfig().getString("scoreboard.economy.formats.currency", "$%.2f");
        this.compactFormat = plugin.getConfig().getString("scoreboard.economy.formats.compact", "auto");
        this.scientificFormat = plugin.getConfig().getString("scoreboard.economy.formats.scientific", "%.2e");
        this.currencySymbol = plugin.getConfig().getString("scoreboard.economy.currency-symbol", "$");

        // Load real-time update settings
        this.realTimeUpdatesEnabled = plugin.getConfig().getBoolean("scoreboard.real-time-updates.enabled", true);
        this.updateOnPlayerJoinQuit = plugin.getConfig().getBoolean("scoreboard.real-time-updates.events.player-join-quit", true);
        this.updateOnEconomyChanges = plugin.getConfig().getBoolean("scoreboard.real-time-updates.events.economy-changes", true);
        this.updateOnLevelChanges = plugin.getConfig().getBoolean("scoreboard.real-time-updates.events.level-changes", true);
        this.updateOnHealthHungerChanges = plugin.getConfig().getBoolean("scoreboard.real-time-updates.events.health-hunger-changes", false);
    ...
    public void setupPlayer(Player player) {
        ...
        Objective objective = scoreboard.registerNewObjective("minetopia", "dummy", translateColors(title));
    ...
    public void removePlayer(Player player) {
        ...
        // Stop update task
        Object task = updateTasks.remove(uuid);
    ...
    /**
     * Update alle scoreboards direct (bijvoorbeeld bij player join/quit events)
     */
    public void updateAllScoreboards() {
        if (!enabled || !realTimeUpdatesEnabled) return;
    ...
    /**
     * Update alle scoreboards voor een specifieke event trigger
     */
    public void triggerUpdate(String eventType) {
        if (!enabled || !realTimeUpdatesEnabled) return;

        switch (eventType.toLowerCase()) {
            case "player-join-quit":
                if (updateOnPlayerJoinQuit) {
                    updateAllScoreboards();
                }
                break;
            case "economy-change":
                if (updateOnEconomyChanges) {
                    updateAllScoreboards();
                }
                break;
            case "level-change":
                if (updateOnLevelChanges) {
                    updateAllScoreboards();
                }
                break;
            case "health-hunger-change":
                if (updateOnHealthHungerChanges) {
                    updateAllScoreboards();
                }
                break;
        }
    ...
    private String processPlaceholders(Player player, String text) {
        ...
        if (Minetopia.getEconomy() != null && economyEnabled) {
            double balance = Minetopia.getEconomy().getBalance(player);
            text = text.replace("%vault_eco_balance_formatted%", formatCurrency(balance, formattedFormat));
            text = text.replace("%vault_eco_balance_short%", formatCurrency(balance, shortFormat));
            text = text.replace("%vault_eco_balance_currency%", formatCurrency(balance, currencyFormat));
            text = text.replace("%vault_eco_balance_compact%", formatCompactCurrency(balance));
            text = text.replace("%vault_eco_balance_scientific%", formatCurrency(balance, scientificFormat));
        } else {
            text = text.replace("%vault_eco_balance_formatted%", "0.00");
            text = text.replace("%vault_eco_balance_short%", "0");
            text = text.replace("%vault_eco_balance_currency%", "$0.00");
            text = text.replace("%vault_eco_balance_compact%", "0");
            text = text.replace("%vault_eco_balance_scientific%", "0.00e+00");
        }
    ...
    private String translateColors(String text) {
        ...
        // 1. Process MiniMessage gradients first (highest priority)
        if (enableGradients) {
            text = processMiniMessageGradients(text);
        }

        // 2. Process gradients first (highest priority)
        if (enableGradients) {
            text = processGradients(text);
        }

        // 3. Process standalone hex colors {#RRGGBB}
        if (enableHexColors) {
            text = processStandaloneHex(text);
        }

        // 4. Process RGB colors rgb(r,g,b)
        if (enableRgbColors) {
            text = translateRgbColors(text);
        }

        // 5. Process HSL colors hsl(hue,sat%,light%)
        text = translateHslColors(text);

        // 6. Process hex colors with &# prefix
        if (enableHexColors) {
            text = translateHexColors(text);
        }

        // 7. Process named colors (&red, &blue, etc.)
        if (enableNamedColors) {
            text = translateNamedColors(text);
        }

        // 8. Finally, process standard Minecraft color codes (&0-&f, &k-&r)
        text = ChatColor.translateAlternateColorCodes('&', text);

        return text;
    }

    private String processMiniMessageGradients(String text) {
        // Pattern: <gradient:#RRGGBB:#RRGGBB>tekst</gradient>
        // Support both 3-digit and 6-digit hex colors
        Pattern gradientPattern = Pattern.compile("<gradient:#([0-9A-Fa-f]{3,6}):#([0-9A-Fa-f]{3,6})>(.+?)</gradient>");
    ...
    private void startUpdateTask(Player player) {
        // Start een eenvoudige periodieke update taak voor deze speler
        Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            if (player.isOnline() && enabled && playerScoreboards.containsKey(player.getUniqueId())) {
                updateScoreboard(player);
            } else {
                // Stop de taak als speler offline is of scoreboard uitgeschakeld
    ...
    private String formatCurrency(double amount, String format) {
        try {
            return String.format(format, amount);
        } catch (Exception e) {
            plugin.getLogger().warning("Failed to format currency with format '" + format + "': " + e.getMessage());
